...
(function(et, ctrl) {
	...
	ctrl.setValidate = function() {
		...
		editValidate.validateRules("group_name",editValidate.REQUIRED,et.message("validate.required", {name:"고객사명"}));
		...
		addValidate.validateRules("group_id",addValidate.REQUIRED,et.message("validate.required", {name:"일련번호"}));
		addValidate.validateRules("group_id",addValidate.NUMBERONLY,et.message("validate.only_num", {name:"일련번호"}));
		addValidate.validateRules("group_name",addValidate.REQUIRED,et.message("validate.required", {name:"고객사명"}));
	...
	ctrl.addEventListener = function () {
		...
		et.setEnterKeyDownEvent("#iptSearchGroupName", _.debounce(self.btnSearchHandler,500),$("#btnSearch"));
		et.setEnterKeyDownEvent("#iptSearchGroupMaster", _.debounce(self.btnSearchHandler,500),$("#btnSearch"));
	...
	ctrl.btnSearchHandler = function (e) {
		...
		if ($(target).is(":visible")) {
			var params = {};
			params.group_name = $("#iptSearchGroupName").val();
			params.group_master = $("#iptSearchGroupMaster").val();
	...
	ctrl.btnDeleteHandler = function () {
		...
		et.alert.showCustomBtn(ETCONST.ALERT_TYPE_CONFIRM, "삭제", et.message("button.delete_confirm"), ["확인", "창닫기"], function(e) {
			...
			params.type = 'group';
			if(params.tenantList !== null) params.tenantList = params.tenantList?.split(",");
			
			new ETService().setSuccessFunction(self.deleteSuccessHandler).callService(self.path + "/delCustMgt", params);
		});
	}
	...
	ctrl.createDatatable = function(postData) {
		...
		var table = et.createDataTableSettings(columns, self.path + "/getCustMgtList", params, self.dataTableDrawCallback);
		table.paging = false;
	...
	ctrl.addSubmitCallbackHandler = function (form) {
		...
		formData.DBTYPE = "I";
		new ETService().setSuccessFunction(self.addSubmitSuccessHandler).callService(self.path + "/setCustMgt", formData);
	}
	...
	ctrl.addSubmitSuccessHandler = function (result) {
		...
		if (result.message === ETCONST.SUCCESS) {
			...
			if (result.data === 0) {
				...
				et.alert.show(ETCONST.ALERT_TYPE_INFO, "", et.message("common.duplicate",{name:"고객사 다이얼러명"}));
			}
		}
	}

 ...
 * @DESC : [관리자웹]  Trunk 게이트웨이 관리
 ******************************************************************************/
(function(et, ctrl) {
	if (_.isObject(et) && et.name === ETCONST.PROJECT_NAME) {
		if (!et.vc || et.vc.name !== "trunkGW") {
			et.vc= ctrl(et);
		}
	} else {
	...
	ctrl.name = "trunkGW";
	ctrl.path = "/gateway/trunkGW";
	ctrl.editFormId = "#editForm";
	ctrl.addFormId = "#addForm";
	ctrl.tableId = "#tbList";
	ctrl.rowData = null;
	ctrl.groupList = null;
	ctrl.gwGroupList = null;
	ctrl.rullBtnSetting = {
				is_write : "#btnAdd"
				, is_delete : "#btnDelete" 
				, is_update : "#btnEdit" 
	...
	ctrl.getGwGroupList = function(){
		var self = et.vc;
		new ETService().setSuccessFunction(self.getTrunkGWGroupSuccessHandler).callService(self.path + "/getTrunkGWGroup", {});
	}
	ctrl.getTrunkGWGroupSuccessHandler = function(result){
		var self = et.vc;
		self.gwGroupList = [];
	...
	ctrl.btnSearchHandler = function (e) {
		...
		if ($(target).is(":visible")) {
			var params = {};
			params.group_gw_id = $("#selGWGroupList").val();
	...
	ctrl.btnAddHandler = function () {
		...
		et.makeSelectOption(self.gwGroupList, {value:"group_gw_id", text:"view_group_gw_name"}, "#selNGWGroupList", "새로운 그룹");  // 조회 gw group
		$("#bxWrite").show();
	}
	...
	ctrl.btnDeleteHandler = function () {
		...
		et.alert.showCustomBtn(ETCONST.ALERT_TYPE_CONFIRM, "삭제", et.message("button.delete_confirm"), ["확인", "창닫기"], function(e) {
			new ETService().setSuccessFunction(self.deleteSuccessHandler).callService(self.path + "/delTrunkGW", params);
		});
	}
	...
	ctrl.createDatatable = function(postData) {
		...
		var table = et.createDataTableSettings(columns, self.path + "/getTrunkGWList", params, self.dataTableDrawCallback);
		table.paging = false;
    ...
    ctrl.tbListRowSelect = function($target, row, col) {
        ...
        et.makeSelectOption(self.gwGroupList, {value:"group_gw_id", text:"view_group_gw_name"}, "#selUGWGroupList", "새로운 그룹",rowData.group_gw_id);  // 조회 gw group
    };
	...
	ctrl.addSubmitCallbackHandler = function (form) {
		...
		formData.isTrunk = "Y";
		formData.DBTYPE = "I";
		new ETService().setSuccessFunction(self.addSubmitSuccessHandler).callService(self.path + "/setTrunkGW", formData);
	}
